...
export const useGameStyles = (breakpoint: Breakpoint) => {
  const getResultStyles = useMemo(() => {
    return (result: string, gameWinner: string) => {
      const baseStyles = breakpoint === 'mobile' 
  ...
  const getHandSelectionStyles = useMemo(() => {
    return (_hand: HANDS, isSelected: boolean, isFocused: boolean = false) => {
      const sizeClasses = breakpoint === 'mobile' 
        ? "w-16 h-16"
        : breakpoint === 'tablet'
        ? "w-24 h-24"
      ...
      if (isSelected && isFocused) {
        selectionStyles = "bg-purple-500 text-white border-2 border-purple-700 ring-2 ring-purple-300";
      } else if (isSelected) {
        selectionStyles = "bg-blue-500 text-white border-2 border-blue-700";
      } else if (isFocused) {
        selectionStyles = "bg-gray-300 text-black border-2 border-gray-500 ring-2 ring-blue-200";
      } else {
  ...
  const getResponsiveTextClasses = useMemo(() => {
    return {
      title: breakpoint === 'mobile' 
        ...
        ? "text-3xl" 
        : "text-4xl",
      score: breakpoint === 'mobile' 
        ? "text-6xl" 
        : breakpoint === 'tablet' 
        ? "text-7xl" 
        : "text-8xl",
      instruction: breakpoint === 'mobile' 
        ? "text-lg" 
        : breakpoint === 'tablet' 
        ? "text-xl" 
